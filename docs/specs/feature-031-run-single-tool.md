...
## Summary
Add a `--tool` flag to `forge run <hook>` so developers can run exactly one
...
seconds per attempt. `forge run pre-commit --tool=phpstan` runs only phpstan
against the currently staged files, giving sub-second feedback loops.

## CLI Interface

```
forge run <hook> --tool <name>           # run one tool
forge run <hook> --tool <name>,<name>    # run multiple tools (comma-separated)
forge run <hook> --group <name>          # run all tools in a group
forge run <hook> --skip-tool <name>      # run all except the named tool(s)
```
...
```
$ forge run pre-commit --tool nonexistent
