# Feature 013: Presets and `forge init` Improvements

## Summary
Extend `forge init` with built-in presets that generate sensible starter
...
4. `--force` overwrites an existing `forge.toml`.
