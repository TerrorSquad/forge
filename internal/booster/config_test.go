...
func TestLoadConfig_InvalidTOML(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, filepath.Join(dir, "booster.toml"), `this is not toml ][[[`)
...
`)
	...
	t.Setenv("BOOSTER_CONFIG", custom)
...
func TestInitConfig_CreatesFile(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(dir, "booster.toml"))
	if err != nil {
		t.Fatalf("ReadFile: %v", err)
	}
	if len(data) == 0 {
		t.Error("expected non-empty booster.toml")
	}
}
...
func TestInitConfig_NoOverwriteWithoutForce(t *testing.T) {
	...
	writeFile(t, filepath.Join(dir, "booster.toml"), "existing")
	if err := InitConfig(false, ""); err == nil {
	...
	for _, preset := range []string{"node", "php", "php-node", "go", "minimal"} {
		t.Run(preset, func(t *testing.T) {
			...
			if err != nil || len(data) == 0 {
				t.Fatalf("expected booster.toml content for preset %q", preset)
			}
		})
	}
}
